...
class Notepad:
	...
	def __init__(self,**kwargs):
		...
		self.__root.title("Неназваный - Заметки")
		...
		self.__thisFileMenu.add_command(label="Создать новую заметку",
		...
		self.__thisFileMenu.add_command(label="Выход",
		...
		self.__thisMenuBar.add_cascade(label="Изменить",
									menu=self.__thisEditMenu)	
		
		self.__thisHelpMenu.add_command(label="О, заметках",
	...
	def __openFile(self):
		...
		if self.__file == "":
			...
			self.__root.title(os.path.basename(self.__file) + " - Заметки")
	...
	def __newFile(self):
		self.__root.title("Без названия - Заметки")
		self.__file = None
		self.__thisTextArea.delete(1.0,END)
	...
	def __saveFile(self):
		...
		if self.__file == None:
			# Save as new file
			self.__file = asksaveasfilename(initialfile='Без названия.txt',
			...
			if self.__file == "":
				...
				self.__root.title(os.path.basename(self.__file) + " - Заметка")
